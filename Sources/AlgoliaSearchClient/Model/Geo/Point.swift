    ...
    public init(from decoder: Decoder) throws {
      ...
      guard rawValue.count == 2 else {
        throw DecodingError.dataCorruptedError(in: container,
                                               debugDescription: """
                                               Decoded string must contain two floating
                                               point values separated by comma character
                                               """)
      }
  ...
  public init(from decoder: Decoder) throws {
    if let arrayForm = try? [DictionaryForm](from: decoder) {
      ...
      let context = DecodingError.Context(codingPath: decoder.codingPath,
                                          debugDescription: """
                                          The format \(encoded) doesn't match
                                          \"22.2268,84.8463\" string or
                                          {\"lat\": 22.2268, \"lng\": 84.8463 } dictionary
                                          """)
