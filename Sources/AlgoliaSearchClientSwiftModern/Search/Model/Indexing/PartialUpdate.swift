...
extension PartialUpdate {
  static func operation(attribute: Attribute, operation: PartialUpdateOperation.Kind, value: JSON) -> Self {
...
extension PartialUpdate: Codable {
  public init(from decoder: Decoder) throws {
    ...
    storage = Dictionary(uniqueKeysWithValues: rawStorage.map { (Attribute(rawValue: $0.key), $0.value) })
  }

  public func encode(to encoder: Encoder) throws {
    let rawKeyValues = storage.map { ($0.key.rawValue, $0.value) }
    try Dictionary(uniqueKeysWithValues: rawKeyValues).encode(to: encoder)
  }
}
